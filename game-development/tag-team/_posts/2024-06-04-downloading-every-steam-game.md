...
1. Finding an existing datasource and downloading it
2. Using a webscraper to manually scrape the data you want from relevant webpages
3. Getting the data from an API

Finding an existing datasource is usually preferable if it's possible.
For example, you can just [download the SOWPODS Scrabble dictionary as plaintext](https://github.com/jesstess/Scrabble/blob/master/scrabble/sowpods.txt).
...
Right now you can load up [the Steam appdetails endpoint](https://store.steampowered.com/api/appdetails?appids=632470)
or [SteamSpy](https://steamspy.com/api.php?request=appdetails&appid=632470) and get up a pretty useful list of details for any given game.
...
I looked to SteamDB for wisdom and [their FAQ page](https://steamdb.info/faq/#how-are-we-getting-this-information)
...
It also references [node-steam-user](https://github.com/DoctorMcKay/node-steam-user), a node module with a function called `getProductInfo`
...
A [guy on Reddit](https://www.reddit.com/r/SteamBot/comments/10dgejp/question_how_to_get_steams_shared_secret_for_totp/)
tried to solve this problem then [wrote a guide](https://gist.github.com/mathielo/8367e464baa73941a075bae4dd5eed90) to step you through
...
I found a lot of threads [like this one](https://steamcommunity.com/discussions/forum/0/1638669204732688825/)
...
This process is simpler, but I almost missed something that made it much easier.
[Here](https://steamapi.xpaw.me/) you can use Steam's public API to make various requests as long as you have a WebAPI key.
...
<br><br>
One way or another, once you have that you can use [this endpoint](https://steamapi.xpaw.me/#IStoreService/GetAppList) to get a list of all Steam games
fulfilling certain criteria.
Of course, you could also be a fool, a rube, a buffoon, even, and use the [identically named endpoint](https://steamapi.xpaw.me/#ISteamApps/GetAppList)
...
Finding out isn't too hard though - I downloaded the [SteamDB list of Steam tags](https://steamdb.info/tags/time/)
...
Using this method you can scrape all 101,017 games on Steam (at time of writing) after running the script for under 10 hours.
...
As I continued to develop this process I was constantly looking for ways to do more stuff with less API calls.
...
-   Scrape tags from [SteamDB](https://steamdb.info/tags/time/)
-   Get list of IDs from [the GetAppList endpoint](https://steamapi.xpaw.me/#IStoreService/GetAppList)
-   For each ID, get the data through the `getProductInfo` function of [node-steam-user](https://github.com/DoctorMcKay/node-steam-user)

However, now that I've done this, you don't have to!
On the off chance you want the exact same data I do,
[you can get it here](https://github.com/Jinan-Dangor/TagTeam-Videogame-Battle/tree/singleplayer-battle)!
